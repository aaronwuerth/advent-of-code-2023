...
impl FromStr for Hand {
    ...
    fn from_str(input: &str) -> Result<Hand, Self::Err> {
        ...
        let mut jokers = 0;
        let mut max = 0;
        let mut index_with_most_cards = 0;
        ...
        for i in 0..5 {
            ...
            if let Some(p) = card_counts.iter().position(|e| e.0 == Some(cards[i])) {
                ...
                if card_counts[p].1 > max {
                    index_with_most_cards = p;
                    max = card_counts[p].1;
                }
            } else if cards[i] == Card::CJ {
                jokers += 1;
            } else {
        ...
        card_counts[index_with_most_cards].1 += jokers;

        let hand_type = if empty_entry == 0 || empty_entry == 1 {
